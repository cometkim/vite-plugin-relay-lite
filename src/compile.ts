...
export function compile(
   ...
   */
  const pattern =
    /(?<prefix>^|[\=\?\:\|\&\,\;\(\[\}\.\>]|\*\/)(?<blank>\s*)graphql`(?<query>[\s\S]*?)`/gm;
  content.replace(
    pattern,
    (match, prefix: string, blank: string, query: string) => {
...
function omitImports(content: MagicString) {
  const pattern =
    /(^[ \t]*(import|const|let|var)\s*{([^}]*?\s*)??)(\s*graphql,\s*|\s*,?\s*graphql\s*)([^}]*\s*?})/gm;
  content.replace(
    pattern,
    (_match, $1: string, _$2, _$3, _omit: string, $5: string) => {
      return $1 + $5;
    }
  );
}
